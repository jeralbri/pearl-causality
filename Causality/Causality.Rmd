...
$P(Z = z \mid do(X = x)) = \sum_z P(Y = y \mid Y = y, X = x)P(Y = y)$

This is true because $P(Z = z \mid do(X = x))$ is what we get after integrating out $Y$.  

But we know from comparing the graphs that $P(Y = y \mid Z = z, X = x)$ and $P(Y = y)$ are the same in both worlds.  Thus, we can identify the causal effect from a world with intervention by the data we get from the the actual world.
...
We can get the _direct effect_ of $X$ on $Z$ if we average over levels of $M$, which is the standard approach to mediation (the _indirect effect_ of $X$ on $Z$ through $M$ is also identifiable depending on $f_z$.
...
How do we deal with this in a manner that allows us to recover the direct effect of $X$ on $Z$?  We now intervene on _both_ $X$ and $W$.

$P(Y=y \mid do(X = x), do(M = m))$.
...
$CDE = P(Y=y \mid do(X = x), do(M = m)) - P(Y=y \mid do(X = x^{\prime}), do(M = m))$
...
There is no path to Gender, so $do(X) = x$, and the CDE is 

$CDE = P(Y=y \mid X = x, do(M = m)) - P(Y=y \mid X = x^{\prime}, do(M = m))$.

The last step is to rewrite the $do(M = m)$ in terms of the observed world.  To block the backdoor path $M \leftarrow W \rightarrow Z$ we need to condition on $Z$.  We are left with:

$$\begin{eqnarray}
CDE = \sum_i \left[P(Y=y \mid X = x, M = m, W = w) - \\
  P(Y=y \mid X = x^{\prime}, M = m, W = w)\right]P(W = w) 
\end{eqnarray}$$
...
2. There exists a set $V_2$ of variables that blocks all backdoor paths from $X$ to $Z$ after deleting all arrows entering $Z$.
...
- Hey, something is eventually significant!  I can graduate and start struggling to pay off my student loans while adjuncting at Mayville State University in North Dakota!
...
The ML practitioner says, "I can simulate a counterfactual by just changing $v_1$ to $v_1^{\prime}$, and the treatment effect is $f_z - f_{z^{\prime}}$.

There are a lot of assumptions behind this statement that, if wrong, lead to the wrong conclusion.

In the simplest case, say that $f_z$ is a linear regression model.  We saw in the previous example that our estimate of $Z$ given a change from $X$ to $X+1$ was different depending on the conditioning set we used.  

The proper conditioning set was determined by the causal ordering of variables.
